...
def save_probabilities_to_file(product_id, category, preference_options, probabilities,
    ...
    with open(f"{output_dir}/{product_id}_{category.lower()}_probabilities.json", "w") as f:
        json.dump(probabilities_data, f, indent=4)

    # Save the probabilities to a CSV file
    with open(f"{output_dir}/{product_id}_{category.lower()}_probabilities.csv", "w", newline='') as f:
        writer = csv.writer(f)
